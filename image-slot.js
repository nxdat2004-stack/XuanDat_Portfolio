...
(() => {
  ...
  function load() {
    ...
    loadP = fetch(STATE_FILE)
      ...
      .catch(() => null)
      .then((j) => {
        // Merge with localStorage — backup for editor + standalone
        let local = null;
        try { const s = localStorage.getItem('image-slots-state'); local = s ? JSON.parse(s) : null; } catch {}
        // localStorage fills in IDs missing from sidecar; sidecar wins on conflict
        return local ? Object.assign({}, local, j || {}) : (j || null);
      })
      .then((j) => {
  ...
  function save() {
    ...
    try { localStorage.setItem('image-slots-state', JSON.stringify(slots)); } catch {}
  ...
  class ImageSlot extends HTMLElement {
    ...
    constructor() {
      ...
      root.innerHTML =
        ...
        '    <div class="cap"></div>' +
        '    <div class="sub" style="display:none">or <u>browse files</u></div></div>' +
    ...
    _render() {
      ...
      // Always editable: omelette saves to sidecar; standalone saves to localStorage
      const editable = true;
      ...
      this._cap.textContent = ph;
      this._cap.style.display = ph ? '' : 'none';
      if (this._sub) this._sub.style.display = ph ? '' : 'none';
